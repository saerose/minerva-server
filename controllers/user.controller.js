...
module.exports = {
  ...
  addBookToLibrary: async (ctx) => {
    let insertBook = {}
    insertBook = {
      title: ctx.request.body.book.volumeInfo.title,
      author: ctx.request.body.book.volumeInfo.authors,
      bookCover: ctx.request.body.book.volumeInfo.imageLinks.thumbnail,
      genre: ctx.request.body.book.volumeInfo.categories
      // reviews: no reviews available in google api
    }
    const newBook = new Book(insertBook)
    console.log(ctx.params.id)
    console.log(ctx.params.book)
